...
export default function HomePage({ 
  ...
  const [initialMainCategories, setInitialMainCategories] = useState<string[]>([]);
  const [initialSubCategories, setInitialSubCategories] = useState<Record<string, string[]>>({});
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setInitialQuotes(quotes);
      setInitialMainCategories(mainCategories);
      setInitialSubCategories(subCategoriesByMain);
      setSelectedMainCategory(mainCategories[0] || 'Todos');
      setIsLoading(false);
    }
    loadData();
  }, []);
  ...
  useEffect(() => {
    if(!isLoading && initialMainCategories.length > 0 && selectedMainCategory === 'Todos') {
      setOpenAccordion(undefined);
    }
  }, [isLoading, initialMainCategories, selectedMainCategory]);
  ...
  }, [searchTerm, selectedMainCategory, selectedSubCategory, initialQuotes, isLoading]);
    ...
    if (isLoading) {
      return (
        <div className="text-center py-16">
          <p className="text-muted-foreground">Carregando frases...</p>
        </div>
      )
    }
